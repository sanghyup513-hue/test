#!/usr/bin/env bash
# =============================================================================
# MLPerf Training v6.0 - Llama 3.1 8B (NeMo)
# Dell XE9780 / B300  -  1 node x 4 GPU  -  NVFP4   (GBS=16, GA=2)
#
#  준비
#    bash mlperf-8b.sh check       자산/config/환경 점검
#    bash mlperf-8b.sh doctor      실행환경 진단 (pyxis/enroot/CONT/최근 잡 실패원인)
#    bash mlperf-8b.sh build       컨테이너 빌드 + sqsh 변환
#    bash mlperf-8b.sh prep        데이터셋 다운로드 (~85GB)
#    bash mlperf-8b.sh inspect     컨테이너 내부 점검 (cuDNN / TransformerEngine)
#
#  실행
#    bash mlperf-8b.sh dryrun      resolved config 만 출력
#    bash mlperf-8b.sh smoke [N]   synthetic data N step 기능 확인 (기본 50)
#    bash mlperf-8b.sh perf  [N]   성능 측정, 실데이터 (기본 200)
#    bash mlperf-8b.sh fp8attn [N] FP8 attention A/B
#    bash mlperf-8b.sh sweep [N]   MICRO_BATCH_SIZE 1/2/4 스윕
#    bash mlperf-8b.sh converge    수렴 런 (log_ppl 3.3, 매우 김)
#    bash mlperf-8b.sh nccl        NCCL 대역폭 테스트만
#
#  결과/정리
#    bash mlperf-8b.sh result      최근 결과 재파싱
#    bash mlperf-8b.sh clean       잔여 잡/컨테이너/GPU 정리
#
# -----------------------------------------------------------------------------
# 성능 지표는 mllog 의 tracked_stats.train_step_time 에서 추출합니다.
#   TFLOPS/GPU = MODEL_TFLOP_PER_SAMPLE(421.59) * GBS / step_time / NGPU
#   tokens/s   = GBS * 8192 / step_time
#
# 이 환경에서 확인된 사항 (기본값에 반영되어 있습니다):
#   1. pyxis 는 로컬 docker 이미지를 못 읽습니다 -> CONT 는 sqsh 경로
#   2. TransformerEngine 이 cuDNN 을 못 찾습니다  -> CUDNN_PATH 명시
#   3. 컨테이너 OMPI 가 PMIx 를 요구, slurm 에 없음 -> NCCL_TEST=0 기본
#   4. 호스트 LD_LIBRARY_PATH 가 컨테이너로 새어듭니다 -> 제출 시 제거
#   5. MAX_STEPS 는 config 를 source 한 "뒤에" 덮어씁니다
#      (config 안에서 바꾸면 OPT_LR_DECAY_STEPS 까지 따라가 LR 궤적 왜곡)
# =============================================================================
set -u

# ---------------------------------------------------------------- 사용자 설정
BASE="${BASE:-/data/lsh}"
REPO="${REPO:-$BASE/training_results_v6.0/Dell/benchmarks/llama31_8b/implementations/nemo}"
DATADIR="${DATADIR:-$BASE/mlperf_training_data}"
LOGDIR="${LOGDIR:-$BASE/mlperf-8b-logs}"
CONT="${CONT:-$BASE/llama31_8b.sqsh}"                     # pyxis 용 squashfs
DOCKER_TAG="${DOCKER_TAG:-mlperf-nvidia:llama31_8b-pyt}"  # docker build 태그

CFG_BASE="config_XE9780_B300_1x4x4xtp1pp1cp1_8b_fp4.sh"
CFG_ATTN="config_XE9780_B300_1x4x4xtp1pp1cp1_8b_fp4_fp8attn.sh"

# 이 이미지는 런타임 cuDNN 패키지만 설치되어 libcudnn.so 심볼릭 링크가 없고,
# cuDNN 이 /usr/local/cuda 가 아니라 /usr/lib/x86_64-linux-gnu 에 있습니다.
# TransformerEngine 의 탐색 순서는
#   CUDNN_HOME -> CUDNN_PATH -> CUDA_HOME -> CUDA_PATH -> /usr/local/cuda
# 이므로 CUDNN_PATH 를 잡아줘야 import 가 통과합니다.
CUDNN_PATH_DEFAULT="${CUDNN_PATH_DEFAULT:-/usr/lib/x86_64-linux-gnu}"

NGPU=4
SEQLEN=8192
TFLOP_PER_SAMPLE=421.59

MODE="${1:-}"; A2="${2:-}"
V="$BASE/8b-$(date +%m%d-%H%M).log"
mkdir -p "$LOGDIR" 2>/dev/null

# ------------------------------------------------------------------- 출력 도구
say(){ printf '%s\n' "$*"; }
ok(){  printf '   OK   %s\n' "$*"; }
ng(){  printf '   --   %s\n' "$*"; }
hr(){  say "--------------------------------------------------"; }
mem(){ nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits 2>/dev/null; }

# ------------------------------------------------------------------- 정리
hard_clean(){
  scancel -u "$(whoami)" >>"$V" 2>&1; sleep 3
  local i pids p
  for i in 1 2 3 4 5 6; do
    pids=$(nvidia-smi --query-compute-apps=pid --format=csv,noheader 2>/dev/null)
    [ -z "$pids" ] && break
    for p in $pids; do kill -9 "$p" 2>/dev/null; done
    sleep 5
  done
  if command -v enroot >/dev/null 2>&1; then
    [ -n "$(enroot list 2>/dev/null)" ] && enroot remove -f $(enroot list) >>"$V" 2>&1
  fi
  for i in 1 2 3 4 5 6; do
    squeue -h -u "$(whoami)" 2>/dev/null | grep -q . || break
    sleep 5
  done
  local busy; busy=$(mem | awk '$1>1000{n++}END{print n+0}')
  if [ "${busy:-0}" -gt 0 ]; then
    ng "GPU 메모리 미해제: [$(mem | tr '\n' ' ')]"
    ng "확인: nvidia-smi --query-compute-apps=pid,used_memory --format=csv"
    return 1
  fi
  ok "GPU 전부 비움 [$(mem | tr '\n' ' ')]   대기잡 $(squeue -h -u "$(whoami)" 2>/dev/null | wc -l)개"
  return 0
}

# ------------------------------------------------------------------- config
load_cfg(){   # $1 = config 파일명
  local c="$REPO/$1"
  [ -f "$c" ] || { ng "config 없음: $c"; return 1; }
  set -a; source "$c"; set +a
  CFG="$1"
  MP=$(( TENSOR_MODEL_PARALLEL * PIPELINE_MODEL_PARALLEL * CONTEXT_PARALLEL ))
  WS=$(( DGXNNODES * DGXNGPU ))
  DP=$(( WS / MP ))
  GBS=$(( MINIBS * DP ))
  GA=$(( MINIBS / MICRO_BATCH_SIZE ))
  VCI_AUTO=$(( (12288 + GBS - 1) / GBS ))
  export CFG MP WS DP GBS GA VCI_AUTO
}

show_cfg(){
  printf " %-22s %s\n" "config"      "$CFG"
  printf " %-22s %s\n" "DGXSYSTEM"   "$DGXSYSTEM"
  printf " %-22s %s\n" "topology"    "${DGXNNODES}node x ${DGXNGPU}gpu = WS ${WS}"
  printf " %-22s %s\n" "parallelism" "TP${TENSOR_MODEL_PARALLEL} PP${PIPELINE_MODEL_PARALLEL} CP${CONTEXT_PARALLEL} -> DP${DP}"
  printf " %-22s %s\n" "batch"       "MINIBS ${MINIBS} x DP ${DP} = GBS ${GBS}   (MBS ${MICRO_BATCH_SIZE}, GA ${GA})"
  printf " %-22s %s\n" "precision"   "FP4=${FP4} recipe=${FP4_RECIPE} FP8_DPA=${FP8_DPA:-False} CUDA_GRAPH=${MCORE_CUDA_GRAPH:-0}"
  printf " %-22s %s\n" "lr/warmup"   "LR ${LR}  WARMUP ${WARMUP_STEPS}  decay_steps ${OPT_LR_DECAY_STEPS}"
  printf " %-22s %s\n" "eval"        "VAL_CHECK_INTERVAL ${VAL_CHECK_INTERVAL} (auto=${VCI_AUTO})  VAL_SAMPLES ${VAL_SAMPLES}"
  printf " %-22s %s\n" "max_steps"   "${MAX_STEPS}"
  [ "$VAL_CHECK_INTERVAL" -ne "$VCI_AUTO" ] && \
    ng "VAL_CHECK_INTERVAL(${VAL_CHECK_INTERVAL}) != ceil(12288/GBS)=${VCI_AUTO} - eval 주기 규칙 불일치"
  [ $(( MINIBS % MICRO_BATCH_SIZE )) -ne 0 ] && \
    ng "MINIBS 가 MICRO_BATCH_SIZE 로 나누어떨어지지 않습니다 (GA 정수 아님)"
  return 0
}

# ------------------------------------------------------------------- 파서
PARSE_PY='
import json,sys,statistics as st
path,ngpu,tflop,seq = sys.argv[1], float(sys.argv[2]), float(sys.argv[3]), int(sys.argv[4])
steps=[]; evals=[]; gbs=None; ga=None; t0=t1=None; status=None
for line in open(path, errors="ignore"):
    i=line.find(":::MLLOG")
    if i<0: continue
    try: d=json.loads(line[i+8:])
    except Exception: continue
    k=d.get("key"); v=d.get("value"); m=d.get("metadata") or {}
    if k=="tracked_stats" and isinstance(v,dict) and "train_step_time" in v:
        t=v["train_step_time"]
        if t and t>0: steps.append(t)
    elif k=="eval_accuracy": evals.append((m.get("samples_count"), v))
    elif k=="global_batch_size": gbs=v
    elif k=="gradient_accumulation_steps": ga=v
    elif k=="run_start": t0=d.get("time_ms")
    elif k=="run_stop":  t1=d.get("time_ms"); status=m.get("status")
warm=min(10,max(0,len(steps)//5))
body=steps[warm:] or steps
out={"n_step":len(steps),"n_used":len(body),"n_warm":warm,"gbs":gbs,"ga":ga,
     "status":status,"evals":evals[-5:],
     "walltime_s":(t1-t0)/1000.0 if (t0 and t1) else None}
if body:
    med=st.median(body); srt=sorted(body)
    out.update({
      "step_median":med,"step_mean":sum(body)/len(body),
      "step_min":srt[0],"step_max":srt[-1],
      "step_p90":srt[int(len(srt)*0.9)-1] if len(srt)>=10 else srt[-1],
      "spread_pct":(srt[-1]-srt[0])/med*100.0,
      "tflops_per_gpu":(tflop*gbs/med/ngpu) if gbs else None,
      "tokens_per_s":(gbs*seq/med) if gbs else None,
      "samples_per_s":(gbs/med) if gbs else None,
    })
print(json.dumps(out))
'

# mllog 에 train_step_time 이 하나라도 있으면 성공으로 봅니다.
has_steps(){ python3 -c "$PARSE_PY" "$1" "$NGPU" "$TFLOP_PER_SAMPLE" "$SEQLEN" 2>/dev/null \
             | python3 -c "import json,sys; sys.exit(0 if json.load(sys.stdin)['n_step']>0 else 1)"; }

# 이번 잡의 mllog 를 DATESTAMP 로 정확히 찾습니다.
find_mllog(){  # $1 = DATESTAMP (없으면 최신)
  local ds="${1:-}"
  if [ -n "$ds" ]; then
    ls -1t "$LOGDIR"/*"${ds}"*.log 2>/dev/null | grep -v mountcheck | grep -E '_[0-9]+\.log$' | head -1
  else
    ls -1t "$LOGDIR"/*.log 2>/dev/null | grep -v mountcheck | grep -E '_[0-9]+\.log$' | head -1
  fi
}

report(){   # $1 = mllog 경로
  local L="${1:-}" J SYN=""
  [ -z "$L" ] && L=$(find_mllog)
  if [ -z "$L" ] || [ ! -f "$L" ]; then ng "파싱할 mllog 가 없습니다 ($LOGDIR)"; return 1; fi
  case "$L" in *_synth_*) SYN=1 ;; esac

  J=$(python3 -c "$PARSE_PY" "$L" "$NGPU" "$TFLOP_PER_SAMPLE" "$SEQLEN" 2>/dev/null)
  say ""
  say "=================================================="
  say " llama31_8b  ${NGPU}GPU (TP1 PP1 CP1 x DP${DP:-4})  GBS ${GBS:-?}"
  say "=================================================="
  printf " %-14s %s\n" "mllog" "$(basename "$L")"
  if [ -z "$J" ]; then ng "파싱 실패"; return 1; fi

  python3 - "$J" "${SYN:-0}" <<'PY'
import json,sys
d=json.loads(sys.argv[1]); syn = sys.argv[2]=="1"
f=lambda k,fmt="%.4f",dflt="N/A": (fmt%d[k]) if d.get(k) is not None else dflt
print(" %-14s %s" % ("측정 step", "%d / %d  (앞 %d step warmup 제외)" % (d["n_used"],d["n_step"],d["n_warm"])))
print(" %-14s %s / %s" % ("GBS / GA", d.get("gbs"), d.get("ga")))
print(" %-14s %s s" % ("step 중앙값", f("step_median")))
print(" %-14s %s s" % ("step 평균",   f("step_mean")))
print(" %-14s %s / %s / %s s   (편차 %s%%)" % ("min/p90/max",
      f("step_min"), f("step_p90"), f("step_max"), f("spread_pct","%.1f")))
print(" %-14s %s" % ("TFLOPS/GPU", f("tflops_per_gpu","%.1f")))
print(" %-14s %s" % ("tokens/s",   f("tokens_per_s","%.0f")))
print(" %-14s %s" % ("samples/s",  f("samples_per_s","%.2f")))
if d.get("walltime_s"): print(" %-14s %.1f s" % ("run 구간", d["walltime_s"]))
print(" %-14s %s" % ("run_stop", d.get("status") or "N/A"))
if d.get("status")=="aborted":
    print("                (MAX_STEPS 도달로 정상 종료. 수렴 런이 아니면 기대값입니다)")
for sc,v in d.get("evals") or []:
    print("   eval_accuracy(log_ppl) @ %s samples : %.4f   (target < 3.3)" % (sc,v))
sp=d.get("spread_pct")
if sp is not None and sp > 8:
    print("")
    print(" !! step 편차 %.1f%% - step 수를 늘려 재측정하세요 (perf 500)" % sp)
if syn:
    print("")
    print(" !! synthetic data 결과입니다. 실측값으로 쓰지 마세요.")
    print("    mock tokenizer 는 vocab 32000 (실제 128256) 이라 embedding /")
    print("    LM head 가 약 0.8B 파라미터 작습니다. 실데이터는 더 느립니다.")
m=d.get("step_median")
print("==================================================")
print(" CODE: V6T.L8B.G4.%s.%s%s" % (("%.4f"%m) if m else "FAIL",
      ("%.1f"%d["tflops_per_gpu"]) if d.get("tflops_per_gpu") else "0",
      ".SYNTH" if syn else ""))
print("==================================================")
PY
  printf " %-14s %s\n" "GPU MiB" "$(mem | tr '\n' ' ')"
  say " 상세: $L"
}

# ------------------------------------------------------------------- 런처
launch(){   # $1=config  $2=태그  $3...=config source 이후 적용할 KEY=VAL
  local cfg="$1"; shift
  local tag="$1"; shift

  load_cfg "$cfg" || return 1
  for kv in "$@"; do export "${kv?}"; done

  export CONT DATADIR LOGDIR
  export NEXP="${NEXP:-1}"
  export SEED="${SEED:-1234}"
  export MLPERF_CLUSTER_NAME="${MLPERF_CLUSTER_NAME:-XE9780}"
  export MLPERF_SCALE="${MLPERF_SCALE:-single_node_4gpu}"
  export CLEAR_CACHES="${CLEAR_CACHES:-0}"      # 1 은 sudo sysctl 필요
  # 컨테이너 OMPI 가 PMIx 를 요구하지만 이 slurm 에는 없습니다. 학습에는 무관하나
  # (학습은 slurm2pytorch 로 랭크를 잡습니다) 로그가 지저분해지므로 기본 off.
  export NCCL_TEST="${NCCL_TEST:-0}"
  export NCCL_LLM_TEST="${NCCL_LLM_TEST:-0}"
  [ "${NO_BIND:-0}" = "1" ] && export BINDCMD=""

  # 이번 잡의 로그 파일명을 특정하기 위해 DATESTAMP 를 직접 정합니다.
  export DATESTAMP="${DATESTAMP:-$(date +'%y%m%d%H%M%S%N')}"

  # sbatch 는 --export=ALL 이 기본이라 제출 쉘 환경이 컨테이너까지 들어갑니다.
  # 호스트 경로가 이미지의 torch/lib, cuda/compat/lib 를 가리면 치명적입니다.
  if [ "${KEEP_HOST_ENV:-0}" != "1" ]; then
    for e in LD_LIBRARY_PATH LD_PRELOAD PYTHONPATH PYTHONHOME CUDA_HOME \
             CPATH LIBRARY_PATH NCCL_ROOT MPI_HOME OPAL_PREFIX; do
      if [ -n "${!e:-}" ]; then say "   호스트 ${e} 제거: ${!e}"; unset "$e"; fi
    done
  fi
  export CUDNN_PATH="${CUDNN_PATH:-$CUDNN_PATH_DEFAULT}"

  say ""
  say "=================================================="
  say " $tag"
  say "=================================================="
  show_cfg
  printf " %-22s %s\n" "CUDNN_PATH" "$CUDNN_PATH"
  printf " %-22s %s\n" "overrides"  "$*${NO_BIND:+ NO_BIND=1}"
  printf " %-22s %s\n" "LOGDIR"     "$LOGDIR"
  hr

  # ---- CONT 검증: pyxis 는 로컬 docker 데몬을 보지 않습니다 ----
  case "$CONT" in
    /*.sqsh|/*.squashfs)
      [ -s "$CONT" ] || { ng "CONT 파일 없음: $CONT"
                          say "      bash $0 build  또는"
                          say "      enroot import -o $CONT dockerd://$DOCKER_TAG"; return 1; }
      ok "CONT = $CONT ($(du -h "$CONT" 2>/dev/null | cut -f1))" ;;
    dockerd://*|docker://*) ok "CONT = $CONT  (매 잡마다 import - sqsh 권장)" ;;
    *)
      ng "CONT 형식 오류: $CONT"
      say "      접두사 없는 이미지명은 pyxis 가 Docker Hub 로 보내 401 이 납니다."
      say "      export CONT=$BASE/llama31_8b.sqsh   (bash $0 build 로 생성)"
      return 1 ;;
  esac

  hard_clean || return 1

  local GRES_OPT=()
  [ "${USE_GRES:-1}" = "1" ] && GRES_OPT=( --gres="gpu:${DGXNGPU}" )

  cd "$REPO" || return 1
  local JOB SOUT
  JOB=$(sbatch --parsable -N "${DGXNNODES}" "${GRES_OPT[@]}" --time="${WALLTIME}" \
        --output="${LOGDIR}/slurm-%j.out" --error="${LOGDIR}/slurm-%j.out" \
        ${SLURM_EXTRA:-} run.sub 2>>"$V")
  if [ -z "$JOB" ]; then ng "sbatch 제출 실패"; tail -20 "$V" | sed 's/^/   /'; return 1; fi
  SOUT="${LOGDIR}/slurm-${JOB}.out"
  say " JobID $JOB   stdout: $SOUT"
  say " 15초마다 상태를 표시합니다."

  local t=0 st
  while :; do
    st=$(squeue -h -j "$JOB" -o '%T' 2>/dev/null)
    [ -z "$st" ] && break
    if [ "$st" = "PENDING" ]; then
      printf '   %s  [PENDING: %s]\n' "$(date +%H:%M:%S)" "$(squeue -h -j "$JOB" -o '%r' 2>/dev/null)"
    else
      printf '   %s  [%s]  GPU MiB [%s]\n' "$(date +%H:%M:%S)" "$st" "$(mem | tr '\n' ' ')"
    fi
    sleep 15; t=$((t+1))
  done
  say " JobID $JOB 종료 (경과 약 $((t/4))분)"
  sleep 5

  # ---- 성공 판정: slurm step 상태가 아니라 mllog 내용으로 판단 ----
  # (NCCL/MPI 테스트 step 이 실패해도 학습은 정상일 수 있습니다. -X 로 할당 단위만 봅니다.)
  local MLLOG; MLLOG=$(find_mllog "$DATESTAMP")
  if [ -n "$MLLOG" ] && has_steps "$MLLOG"; then
    say ""
    sacct -X -n -j "$JOB" -o State,ExitCode,Elapsed 2>/dev/null \
      | sed 's/^/   slurm: /' | head -1
    report "$MLLOG"
    unset DATESTAMP
    return 0
  fi

  ng "학습 step 이 기록되지 않았습니다."
  say ""
  say "[slurm 상태]"
  sacct -X -j "$JOB" --format=JobID,State,ExitCode,Elapsed,Reason%30 2>/dev/null | sed 's/^/   /'
  say ""
  say "[slurm stdout 마지막 50줄]  $SOUT"
  if [ -f "$SOUT" ]; then
    tail -50 "$SOUT" | sed 's/^/   /'
    say ""
    say "[치명적 에러 후보]  (MPI_Init / pmix / cpuset.mems 는 무해합니다)"
    grep -inE 'Traceback|RuntimeError|ImportError|ModuleNotFound|CUDA (error|out of memory)|OutOfMemory|shared object not found|401 Unauthorized|couldn.t start container' \
      "$SOUT" 2>/dev/null | head -12 | sed 's/^/   /'
  else
    ng "stdout 없음: $SOUT"
  fi
  say ""
  say " 점검:  bash $0 doctor    /    bash $0 inspect"
  unset DATESTAMP
  return 1
}

# =============================================================================
case "$MODE" in

clean) hard_clean; squeue -u "$(whoami)" 2>/dev/null ;;

result)
  load_cfg "${A2:-$CFG_BASE}" >/dev/null 2>&1
  report ;;

check)
  say "MLPerf Training v6.0  Llama 3.1 8B  사전점검"
  say ""
  say "[경로]"
  [ -d "$REPO" ] && ok "REPO $REPO" || ng "REPO 없음: $REPO"
  for f in run.sub run_and_time.sh config_mounts.sh pretrain.py conf/custom.yaml; do
    [ -f "$REPO/$f" ] && ok "$f" || ng "누락: $f"
  done
  say ""
  say "[config]"
  for c in "$CFG_BASE" "$CFG_ATTN"; do
    if [ -f "$REPO/$c" ]; then
      bash -n "$REPO/$c" 2>/dev/null && ok "$c" || ng "$c 문법 오류"
    else ng "$c 없음 - REPO 에 복사하세요"; fi
  done
  for c in config_common.sh config_common_fp4.sh config_common_cg.sh \
           config_common_8b.sh config_common_fp8attn.sh; do
    [ -f "$REPO/$c" ] || ng "의존 파일 없음: $c"
  done
  say ""
  say "[데이터셋]  $DATADIR/8b"
  if [ -d "$DATADIR/8b" ]; then
    for f in c4-train.en_6_text_document.bin c4-train.en_6_text_document.idx \
             c4-validation-91205-samples.en_text_document.bin \
             c4-validation-91205-samples.en_text_document.idx \
             tokenizer/tokenizer.json tokenizer/tokenizer_config.json; do
      [ -s "$DATADIR/8b/$f" ] && ok "$f ($(du -h "$DATADIR/8b/$f" | cut -f1))" || ng "누락: $f"
    done
  else ng "없음 - bash $0 prep"; fi
  say "  여유 $(df -BG --output=avail "$DATADIR" 2>/dev/null | tail -1 | tr -d ' ')  (데이터셋 ~90GB)"
  say ""
  say "[컨테이너]"
  case "$CONT" in
    /*.sqsh|/*.squashfs) [ -s "$CONT" ] && ok "$CONT ($(du -h "$CONT" | cut -f1))" \
                                        || ng "$CONT 없음 - bash $0 build" ;;
    *) ng "CONT 형식 오류: $CONT  (sqsh 절대경로 필요)" ;;
  esac
  say ""
  say "[SLURM / GPU]"
  command -v sbatch >/dev/null 2>&1 && ok "sbatch" || ng "sbatch 없음"
  srun --help 2>/dev/null | grep -q container-image && ok "pyxis" || ng "pyxis 없음"
  command -v enroot >/dev/null 2>&1 && ok "enroot" || ng "enroot 없음"
  say "  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1) x $(nvidia-smi -L 2>/dev/null | wc -l)"
  say "  GPU MiB [$(mem | tr '\n' ' ')]   대기잡 $(squeue -h -u "$(whoami)" 2>/dev/null | wc -l)개"
  say ""
  if load_cfg "$CFG_BASE" 2>/dev/null; then say "[해석된 config]"; show_cfg; fi
  say ""
  say " 다음:  bash $0 dryrun  ->  bash $0 smoke  ->  bash $0 perf"
  ;;

doctor)
  say "실행환경 진단"
  say ""
  say "[SLURM]"
  command -v sbatch >/dev/null 2>&1 && ok "sbatch" || ng "sbatch 없음"
  command -v sacct  >/dev/null 2>&1 && ok "sacct"  || ng "sacct 없음 (종료상태 확인 제한)"
  sinfo -h -o '   node=%n state=%t cpus=%c gres=%G' 2>/dev/null | sed 's/^/  /'
  say ""
  say "[pyxis / enroot]"
  srun --help 2>/dev/null | grep -q container-image && ok "pyxis" || ng "pyxis 없음"
  command -v enroot >/dev/null 2>&1 && ok "enroot $(enroot version 2>/dev/null)" || ng "enroot 없음"
  say ""
  say "[CONT]  $CONT"
  case "$CONT" in
    /*.sqsh|/*.squashfs) [ -s "$CONT" ] && ok "존재 ($(du -h "$CONT" | cut -f1))" || ng "파일 없음" ;;
    dockerd://*|docker://*) ok "URI 형식" ;;
    *) ng "pyxis 는 로컬 docker 이미지를 못 읽습니다 -> sqsh 절대경로 필요" ;;
  esac
  say ""
  say "[호스트 환경 - 컨테이너로 새어 들어갑니다]"
  for e in LD_LIBRARY_PATH LD_PRELOAD PYTHONPATH CUDA_HOME CUDNN_PATH; do
    printf '   %-18s [%s]\n' "$e" "${!e:-unset}"
  done
  say "   (런처가 제출 시 자동 제거합니다. KEEP_HOST_ENV=1 로 유지)"
  say ""
  say "[GPU]"
  nvidia-smi -L 2>/dev/null | sed 's/^/   /'
  say "   요청 예정: ${NGPU}장 / 보이는 GPU $(nvidia-smi -L 2>/dev/null | wc -l)장"
  say ""
  say "[최근 잡]"
  sacct -X -n -u "$(whoami)" -S "$(date -d '2 days ago' +%Y-%m-%d)" \
        --format=JobID%10,State%12,ExitCode%8,Elapsed%10,Reason%28 2>/dev/null \
    | tail -5 | sed 's/^/   /'
  say ""
  say "[최근 slurm stdout]"
  for f in $(ls -1t "$LOGDIR"/slurm-*.out "$REPO"/slurm-*.out 2>/dev/null | head -1); do
    say "   --- $f"
    grep -inE 'Traceback|RuntimeError|ImportError|shared object not found|401 Unauthorized|couldn.t start container|out of memory' \
      "$f" 2>/dev/null | head -10 | sed 's/^/     /' || say "     (치명적 에러 없음)"
  done
  ;;

inspect)
  case "$CONT" in
    /*.sqsh|/*.squashfs) [ -s "$CONT" ] || { ng "CONT 없음: $CONT"; exit 1; } ;;
    dockerd://*|docker://*) ;;
    *) ng "CONT 형식 오류: $CONT"; exit 1 ;;
  esac
  say "컨테이너 내부 점검  CONT=$CONT"
  say ""
  env -u LD_LIBRARY_PATH -u LD_PRELOAD -u PYTHONPATH -u CUDA_HOME -u CUDNN_PATH \
  srun --ntasks=1 -N1 --time=10 --container-image="$CONT" \
       --no-container-mount-home --container-remap-root --container-writable \
       bash -c '
    echo "  LD_LIBRARY_PATH = [$LD_LIBRARY_PATH]"
    echo "  CUDNN_VERSION   = [${CUDNN_VERSION:-unset}]"
    echo "  --- libcudnn 파일 (버전 없는 libcudnn.so 가 있는지 확인)"
    ls -l /usr/lib/x86_64-linux-gnu/libcudnn.so* 2>&1 | head -5
    echo "  --- dpkg"
    dpkg -l 2>/dev/null | grep -i cudnn || echo "    (cudnn 패키지 없음 - 재빌드 필요)"
    echo "  --- CUDNN_PATH 없이 import"
    python -c "import transformer_engine.pytorch; print(\"    OK\")" 2>&1 | tail -1
    echo "  --- CUDNN_PATH=/usr/lib/x86_64-linux-gnu 로 import"
    CUDNN_PATH=/usr/lib/x86_64-linux-gnu \
      python -c "import transformer_engine.pytorch; print(\"    OK\")" 2>&1 | tail -1
    python -c "import torch; print(\"  torch\", torch.__version__, \"cuda\", torch.version.cuda)"
  '
  say ""
  say " 두 번째 import 만 OK 면 런처가 이미 자동 처리합니다 (CUDNN_PATH 기본값)."
  say " 둘 다 실패하면 Dockerfile 에 아래를 넣고 재빌드하세요:"
  say "   RUN ln -sf /usr/lib/x86_64-linux-gnu/libcudnn.so.9 \\"
  say "              /usr/lib/x86_64-linux-gnu/libcudnn.so"
  ;;

build)
  say "컨테이너 빌드  |  로그 $V"
  cd "$REPO" || exit 1
  say "  base: $(grep -m1 '^ARG FROM_IMAGE_NAME' Dockerfile | cut -d= -f2)"
  say "  nvcr.io 로그인이 없으면 여기서 401 이 납니다:"
  say "    echo \$NGC_API_KEY | docker login nvcr.io -u '\$oauthtoken' --password-stdin"
  say ""
  docker build -t "$DOCKER_TAG" ${FROM_IMAGE_NAME:+--build-arg FROM_IMAGE_NAME=$FROM_IMAGE_NAME} \
    --build-arg GIT_COMMIT_ID="$(git rev-parse --short HEAD 2>/dev/null || echo na)" . \
    2>&1 | tee -a "$V" | tail -20
  docker image inspect "$DOCKER_TAG" >/dev/null 2>&1 || { ng "빌드 실패 - $V"; exit 1; }
  ok "빌드 완료: $DOCKER_TAG"
  say ""
  say "sqsh 변환 (pyxis 는 로컬 docker 이미지를 못 읽습니다). 10~20분."
  export ENROOT_TEMP_PATH="${ENROOT_TEMP_PATH:-$BASE/enroot-tmp}"
  mkdir -p "$ENROOT_TEMP_PATH"
  say "  ENROOT_TEMP_PATH=$ENROOT_TEMP_PATH (여유 $(df -BG --output=avail "$ENROOT_TEMP_PATH" 2>/dev/null | tail -1 | tr -d ' '))"
  rm -f "$CONT"
  enroot import -o "$CONT" "dockerd://$DOCKER_TAG" 2>&1 | tee -a "$V" | tail -8
  if [ -s "$CONT" ]; then
    ok "sqsh 생성: $CONT ($(du -h "$CONT" | cut -f1))"
    say " ~/.bashrc 에 추가:  export CONT=$CONT"
  else ng "sqsh 변환 실패 - $V (디스크/도커 권한 확인)"; fi
  ;;

prep)
  say "데이터셋 준비 (~85GB)  |  DATADIR=$DATADIR  로그 $V"
  mkdir -p "$DATADIR"
  cd "$REPO" || exit 1
  DATADIR="$DATADIR" bash data_scripts/download_8b.sh 2>&1 | tee -a "$V" | tail -20
  say ""; ls -lh "$DATADIR/8b" 2>/dev/null | sed 's/^/   /'
  say ""; say " 검증: bash $0 check"
  ;;

nccl)
  # PMIx 가 되는 환경에서만 의미가 있습니다.
  launch "$CFG_BASE" "NCCL 대역폭 테스트만" \
    RUN_ONLY_NCCL=1 NCCL_TEST=1 NCCL_TEST_WALLTIME=10 \
    USE_SYNTHETIC_DATA=1 VERIFY_MOUNTS=0 CHECK_COMPLIANCE=0 \
    WALLTIME_RUNANDTIME=15 WALLTIME=20
  ;;

dryrun)
  launch "$CFG_BASE" "DRYRUN - resolved config 출력" \
    PRINT_CONFIG_ONLY=True USE_SYNTHETIC_DATA=1 VERIFY_MOUNTS=0 \
    MAX_STEPS=1 CHECK_COMPLIANCE=0 WALLTIME_RUNANDTIME=20 WALLTIME=25
  say " stdout 에서 model.global_batch_size / trainer.val_check_interval 확인"
  ;;

smoke)
  launch "$CFG_BASE" "SMOKE - synthetic data ${A2:-50} step (기능 확인, 성능 무의미)" \
    USE_SYNTHETIC_DATA=1 VERIFY_MOUNTS=0 CHECK_COMPLIANCE=0 \
    MAX_STEPS="${A2:-50}" LOG_EVERY_N_STEPS=1 LOG_METRICS=DELTA \
    WALLTIME_RUNANDTIME=40 WALLTIME=45
  ;;

perf)
  # VAL_CHECK_INTERVAL(768) > MAX_STEPS 이면 eval 이 돌지 않아 순수 step time 만 측정됩니다.
  launch "$CFG_BASE" "PERF - 실데이터 ${A2:-200} step" \
    MAX_STEPS="${A2:-200}" LOG_EVERY_N_STEPS=1 LOG_METRICS=DELTA \
    CHECK_COMPLIANCE=0 WALLTIME_RUNANDTIME=60 WALLTIME=70
  ;;

fp8attn)
  launch "$CFG_ATTN" "PERF (FP8 attention) - ${A2:-200} step  <- baseline 과 A/B" \
    MAX_STEPS="${A2:-200}" LOG_EVERY_N_STEPS=1 LOG_METRICS=DELTA \
    CHECK_COMPLIANCE=0 WALLTIME_RUNANDTIME=60 WALLTIME=70
  ;;

sweep)
  # MICRO_BATCH_SIZE 는 GBS 를 바꾸지 않는 유일한 성능 노브입니다 (README_8b.md).
  # MINIBS=4 이므로 1/2/4 만 GA 가 정수. MBS=4 는 GA=1 로 Dell 1x8x2 와 같은 조건.
  S="${A2:-150}"
  RES=()
  for MBS in 1 2 4; do
    say ""; say "########## MICRO_BATCH_SIZE=$MBS  (GA=$((4/MBS))) ##########"
    if launch "$CFG_BASE" "SWEEP MBS=${MBS}" \
        MICRO_BATCH_SIZE="$MBS" MAX_STEPS="$S" LOG_EVERY_N_STEPS=1 LOG_METRICS=DELTA \
        CHECK_COMPLIANCE=0 WALLTIME_RUNANDTIME=50 WALLTIME=60; then
      L=$(find_mllog)
      T=$(python3 -c "$PARSE_PY" "$L" "$NGPU" "$TFLOP_PER_SAMPLE" "$SEQLEN" 2>/dev/null \
          | python3 -c "import json,sys;d=json.load(sys.stdin);print('%.4f %.1f'%(d.get('step_median') or 0,d.get('tflops_per_gpu') or 0))")
      RES+=("MBS=$MBS GA=$((4/MBS))  step=$(echo $T|cut -d' ' -f1)s  TFLOPS/GPU=$(echo $T|cut -d' ' -f2)")
    else
      RES+=("MBS=$MBS  실패")
    fi
  done
  say ""
  say "=================================================="
  say " MICRO_BATCH_SIZE 스윕 결과 (GBS=16 불변)"
  say "=================================================="
  for r in "${RES[@]}"; do say "   $r"; done
  say "=================================================="
  ;;

converge)
  say "수렴 런: TARGET_LOG_PPL=3.3. 4 GPU(GBS=16)에서는 수십~수백 시간 규모입니다."
  say "중단 후 재개되도록 ENABLE_RERUNS/SHARE_RERUNS 를 켭니다."
  launch "$CFG_BASE" "CONVERGE - log_ppl 3.3 목표" \
    ENABLE_RERUNS=1 SHARE_RERUNS=1 STORE_CKPTS_IN_LOGDIR=1 WALLTIME_EXIT_MINUTES=10
  ;;

*) sed -n '4,28p' "$0" ;;
esac
